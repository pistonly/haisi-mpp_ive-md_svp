...
int main(int argc, char *argv[]) {
  ...
  // std::string high_resolution_path = "./data/input/md/1080p_yuv420sp.bin";
  std::string omPath = "/home/liuyang/Documents/haisi/ai-sd3403/"
                       "ai-sd3403/models/yolov8n_320x320_rpn_original.om";
  ...
  const int IMAGE_SIZE2 = OT_SAMPLE_MD_WIDTH * OT_SAMPLE_MD_HEIGHT * 1.5 * 4;
  ...
  if (inputFile2.fail()) {
    std::cerr << "Failed to read image2 " << std::endl;
    return 1;
  }
  inputFile2.close();

  for (int i = 0; i < numberOfImages; ++i) {
    std::cout << "image_id: " << i << std::endl;

    auto start_time = std::chrono::high_resolution_clock::now();
    md.process(v_image[i % 4].data(), &blob);
    ...
    std::cout << "here-" << std::endl;
    // merge_rois(img_high.data(), &blob, merged_roi, top_lefts, 4.0f, 4.0f,
    // 1080,
    //            1920, merged_hw, merged_hw);
...
void merge_rois(const unsigned char *img, ot_ive_ccblob *p_blob,
  ...
  const int rgn_num =
      std::min(static_cast<int>(p_blob->info.bits.rgn_num), 100);
