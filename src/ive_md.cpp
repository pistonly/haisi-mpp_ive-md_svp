...
IVE_MD::~IVE_MD() {
  for (td_u16 i = 0; i < OT_SAMPLE_MD_SRC_NUM; ++i) {
    sample_svp_mmz_free(m_mdstep.img[i].phys_addr[0],
                        m_mdstep.img[i].virt_addr[0]);
  }
  sample_svp_mmz_free(m_mdstep.diff.phys_addr[0], m_mdstep.diff.virt_addr[0]);
  sample_svp_mmz_free(m_mdstep.sad_zeros.phys_addr[0],
                      m_mdstep.sad_zeros.virt_addr[0]);
  sample_svp_mmz_free(m_mdstep.sad.phys_addr[0], m_mdstep.sad.virt_addr[0]);
  sample_svp_mmz_free(m_mdstep.sad_thres.phys_addr[0],
                      m_mdstep.sad_thres.virt_addr[0]);
...
int IVE_MD::process_core(ot_ive_ccblob *blob) {
  ...
  // output blob
  ot_ive_ccblob *blob_tmp =
      sample_svp_convert_addr_to_ptr(ot_ive_ccblob, m_mdstep.blob.virt_addr);
...
int IVE_MD::process(ot_video_frame_info &frame, ot_ive_ccblob *blob) {
  ...
  m_ret = sample_common_ive_dma_image(&frame, &m_mdstep.img[m_current_img],
                                      is_instant);
  return process_core(blob);
}
