...
void resize_yuv420(std::vector<unsigned char> &outputYUV, int &outputOffsetX,
  ...
  // 源图像数据指针
  const unsigned char *src = inputFrame.data();
  ...
  outputOffsetX = offsetX;
  outputOffsetY = offsetY;

  // 4. 分配输出缓冲区，大小为 outputWidth * outputHeight * 1.5
  outputYUV.resize(outputWidth * outputHeight * 3 / 2);

  // 初始化缓冲区为黑色 (Y=0, UV=128)
  size_t outputYsize = outputWidth * outputHeight;
  std::fill(outputYUV.begin(), outputYUV.begin() + outputYsize, 0);
  std::fill(outputYUV.begin() + outputYsize, outputYUV.end(), 128);
  unsigned char *dst = outputYUV.data();

  // 5. 处理 Y 分量
  const unsigned char *srcY = src; // 源 Y 分量起始地址
  unsigned char *dstY = dst; // 目标 Y 分量起始地址
  for (int row = 0; row < scaledHeight; ++row) {
    for (int col = 0; col < scaledWidth; ++col) {
      // 源图像对应的采样点
      int srcRow = static_cast<int>(row / scale);
      int srcCol = static_cast<int>(col / scale);
      unsigned char pixel = srcY[srcRow * inputWidth + srcCol];

      // 目标图像偏移量
      int dstRow = row + offsetY;
      int dstCol = col + offsetX;
      dstY[dstRow * outputWidth + dstCol] = pixel;
    }
  }

  // 6. 处理 UV 分量 (NV12 格式: UV 交织, 分辨率为 inputWidth/2 x inputHeight/2)
  const unsigned char *srcUV = src + inputWidth * inputHeight;
  unsigned char *dstUV = dst + outputWidth * outputHeight;
  for (int row = 0; row < scaledHeight / 2; ++row) {
    for (int col = 0; col < scaledWidth / 2; ++col) {
      // 源图像对应的采样点
      int srcRow = static_cast<int>(row / scale);
      int srcCol = static_cast<int>(col / scale);
      int srcIdx = srcRow * (inputWidth / 2) + srcCol;

      // 目标图像偏移量
      int dstRow = row + offsetY / 2;
      int dstCol = col + offsetX / 2;
      int dstIdx = dstRow * (outputWidth / 2) + dstCol;

      // 拷贝 UV 值 (两个字节, U 和 V)
      dstUV[dstIdx * 2] = srcUV[srcIdx * 2];         // U
      dstUV[dstIdx * 2 + 1] = srcUV[srcIdx * 2 + 1]; // V
    }
  }
}
...
void resize_yuv420(std::vector<unsigned char> &outputYUV, int &outputOffsetX,
  ...
  td_u32 inputSize =
      inputHeight * inputWidth * 3 / 2; // YUV420SP (NV12) 数据大小

  // 1. 通过物理地址映射到用户态指针
  td_void *frame_data =
      ss_mpi_sys_mmap_cached(frame->video_frame.phys_addr[0], inputSize);
  if (frame_data == NULL) {
    sample_print("mmap failed!\n");
    return;
  }
  ...
  // 源图像数据指针
  unsigned char *src = reinterpret_cast<unsigned char *>(frame_data);
  ...
  // 5. 处理 Y 分量
  unsigned char *srcY = src; // 源 Y 分量起始地址
  unsigned char *dstY = dst; // 目标 Y 分量起始地址
  for (int row = 0; row < scaledHeight; ++row) {
    for (int col = 0; col < scaledWidth; ++col) {
      // 源图像对应的采样点
  ...
  // 6. 处理 UV 分量 (NV12 格式: UV 交织, 分辨率为 inputWidth/2 x inputHeight/2)
  unsigned char *srcUV = src + inputWidth * inputHeight;
  unsigned char *dstUV = dst + outputWidth * outputHeight;
  for (int row = 0; row < scaledHeight / 2; ++row) {
    for (int col = 0; col < scaledWidth / 2; ++col) {
      // 源图像对应的采样点
  ...
  // 7. 解映射
  ss_mpi_sys_munmap(frame_data, inputSize);
}
...
void copy_split_yuv420_from_frame(
    std::vector<std::vector<unsigned char>> &outputImageDatas,
    const std::vector<unsigned char> &srcFrame, const int srcHeight, const int srcWidth) {
  td_u32 size = srcHeight * srcWidth * 3 / 2; // 对于YUV420格式，大小为宽*高*1.5

  const unsigned char *frame_data = srcFrame.data();

  // Calculate sizes
  int ySize = srcWidth * srcHeight;
  int uvSize = ySize / 2; // Since UV are interleaved in YUV420sp

  int yHalfWidth = srcWidth / 2;
  int yHalfHeight = srcHeight / 2;
  int yHalfSize = yHalfWidth * yHalfHeight;
...
void blob_to_xyxy(const ot_ive_ccblob *p_blob,
                  ...
                  const int max_size) {
  blob_xyxy.clear();
  const int rgn_num = static_cast<int>(p_blob->info.bits.rgn_num);
  for (int i = 0; i < rgn_num; ++i) {
    auto &rgn = p_blob->rgn[i];
    float rgn_x0 = scale * static_cast<float>(rgn.left);
    float rgn_y0 = scale * static_cast<float>(rgn.top);
    float rgn_x1 = scale * static_cast<float>(rgn.right);
    float rgn_y1 = scale * static_cast<float>(rgn.bottom);
    if (std::max(rgn_x1 - rgn_x0, rgn_y1 - rgn_y0) > max_size) {
      continue;
    }
...
void merge_rois(const unsigned char *img,
                ...
                int merged_roi_H, int merged_roi_W, int max_roi_num) {
  top_lefts.clear();
  // img and merged_rois are YUV format images.
  const int roi_size = 32;
  const int roi_size_half = roi_size / 2;
